...
journal_data = pd.read_excel("C:\\Users\\BFI User\\Desktop\\JPE_2000-2022.xlsx")
...
for index, row in journal_data.iterrows():
    ...
    m = soup.find_all('ul', {"class": "rlist article__navbar__col article__navbar__row article__navbar__main-options"})
    m = m[0].find_all('a', {'class': 'ctrl--primary ctrl'})
    tail = m[0].attrs['href']
    x = 'https://www.journals.uchicago.edu' + tail
    x = x.replace('epdf', 'pdf')
    x += '?download=true'
