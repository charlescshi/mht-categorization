...
journal_data = pd.read_excel("C:\\Users\\BFI User\\Desktop\\Econometrica_2000-2022.xlsx")
...
for index, row in journal_data.iterrows():
    ...
    m = soup.find_all('div', {"class": "right"})
    m = m[0].find_all('a', {'class': 'green_button', 'target': '_blank'})
    tail = m[0].attrs['href']
    x = 'https://www.econometricsociety.org' + tail

    driver.get(x)

    x = driver.current_url.replace('pdf', 'pdfdirect')
